'use strict';

window.translateFR = {
    SEARCH: 'Recherche',
    ORIGIN: 'Commande',
    NAME: 'Numéro d\'expédition',
    PARTNER: 'Client',
    CARRIER: 'Transporteur',
    EMAIL: 'Email',
    PAYMENT_METHOD: 'methode de paiement',
    ADD_ALL: 'Tout ajouter',
    EDIT: 'Editer',
    TO_SEND: 'A traiter',
    DONE: 'Traité',
    ADD_ONE: '+ 1',
    CANCEL: 'Annuler',
    NO_ORDER_FOUND: 'Aucune commande trouvé, veuillez resaisir le numéro',
    USER_ERROR: 'Erreur utilisateur',
    NO_PRODUCT_FOUND: 'Le code barre saisi ne correspond pas à un produit de la commande',
    NOTE: 'Commentaire',
    PAYMENT_DONE: 'Le règlement à bien été effectué',
    PAYMENT_PROCESSING: 'Capture en cours',
    PAYMENT_FAIL: 'Capture échouée',
    PAYMENT_MISSING: 'Aucun réglement n\'a été fait',
    VALIDATE_AND_PRINT: 'Validation et impression en cours...',
    PAYMENT: 'Paiement information',
    CAN_NOT_PROCESS_PICKING: 'L\'expédition ne peut pas être traité',
    SET_CARRIER_ORDER_AS_PREPARED: 'Le transporteur sélectionné ne peut pas être traité via l\'application. Veuillez mettre la commande dans le bac des {{carrier_name}}. Impression du Bon de livraison en cours ...',
    SET_PAID_ORDER_AS_PREPARED: 'La commande n\'est pas payé. Veuillez la mettre dans le bac des commandes préparés',
    PARTIAL: 'Partiel',
    MANUAL_PRINT_PROCESSING: 'Impression du bon {{ number }} en cours',
    MANUAL_PRINT_FAIL: 'Erreur d\'impression veuillez vérifier la connection du matériel',
    MANUAL_PRINT_ERROR: 'Erreur d\'impression veuillez vérifier la connection et le paramétrage du matériel',
    MANUAL_NO_PICKING: 'Aucune Expédition trouvé pour le numéro {{ number }}',
    DIRECT_PRINT: 'Impression direct',
    PRINT_HELP: 'Veuillez scanner dans la barre de recherche le numéro de bon à réimprimer',
    BLOCKED: 'Blocké',

}
